...
function zumbroich_perf(ran)
    @testset "zumbroich perf" begin
        ...
        plain = [
            (@timed sum(first, Cyclotomics.zumbroich_plain(i)))[2] for i in ran
        ]
        ...
        compl = [
            (@timed sum(first, Cyclotomics.zumbroich_basis(i)))[2] for i in ran
        ]
        ...
        direc =
            [(@timed length(Cyclotomics.zumbroich_direct(i)))[2] for i in ran]
        @info "GAP style: mean and variance over $ran" μ =
            round(mean(direc), sigdigits = 4) σ =
